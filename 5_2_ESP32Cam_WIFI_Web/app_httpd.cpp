...
// actuator and servo pan & tilt
const int Servo_tilt      = 30;  
const int Servo_pan       = 40;
const int Servo_dolly     = 41;
const int Actuator_up     = 225;
...
static esp_err_t cmd_handler(httpd_req_t *req)
{
    ...
    else if(!strcmp(variable, "pan"))
    {
     ...
     // ===== SERVO DOLLY =====
    else if(!strcmp(variable, "dolly"))
    {
        if(val < 0) val = 0;
       else if(val > 180) val = 180;

        txdata[1] = Servo_dolly;
        txdata[2] = val;
        Serial.write(txdata,4);
    }
...
<html>
    <head>
        ...
        <title>MAKERLAB MECANUM ROBOT</title>
        <style>
            *{
                padding: 0; margin: 0;
    ...
    <body>
        ...
        <h1 class="tITULO">MAKERLAB MECANUM ROBOT</h1>
        ...
        <h3 style="text-align:center;margin-top:20px;">Camera & Actuator Control</h3>

        <div class="cont_flex">  
            <div style="display: flex;align-items: center;">Pan <input type="range" id="pan" min="0" max="180" value="90" onchange="try{fetch(document.location.origin+'/control?var=pan&val='+this.value);}catch(e){}"></div>
        </div>
        ...
        <!-- ====== DOLLY CONTROL ====== -->
        <h3 style="text-align:center;margin-top:20px;">Dolly Control</h3>
        <div class="cont_flex">
            <button onclick="fetch('/control?var=dolly&val=0')"
                style="width:100px;height:40px;">Left</button>

            <button onclick="fetch('/control?var=dolly&val=90')"
                style="width:100px;height:40px;">Center</button>

            <button onclick="fetch('/control?var=dolly&val=180')"
                style="width:100px;height:40px;">Right</button>
        </div>
...
// PAN & TILT SLIDERS
function setPan(val){
    fetch(`/control?var=pan&val=${val}`);
}

function setTilt(val){
    fetch(`/control?var=tilt&val=${val}`);
}
